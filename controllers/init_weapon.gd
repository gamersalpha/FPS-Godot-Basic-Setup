...
func load_weapon() -> void:
	...
	idle_sway_adjustement = WEAPON_TYPE.idle_sway_adjustment
...
func sway_weapon(delta) -> void:
	if not Engine.is_editor_hint():
		...
		var sway_random_adjusted : float = sway_random * idle_sway_adjustement

		time += delta * ( 1 + sway_speed + sway_random)
